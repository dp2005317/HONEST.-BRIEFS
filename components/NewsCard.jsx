      ...
      <div className={`relative w-full overflow-hidden border mb-6 ${featured ? 'aspect-[21/9] sm:aspect-[16/6]' : 'aspect-video'}`} style={{ borderColor: 'var(--border-ink)', backgroundColor: 'var(--skeleton-bg)', borderRadius: 'var(--border-radius)' }}>
        {image ? (
          ...
          <div className="w-full h-full flex items-center justify-center opacity-20">
            <span className="font-serif italic text-sm">Editorial illustration missing</span>
          </div>
        )}
        
        {/* Source Badge */}
        <div className="absolute bottom-4 left-4 px-3 py-1 text-[9px] font-black uppercase tracking-widest" style={{ backgroundColor: 'var(--badge-bg)', border: `2px solid var(--badge-border)`, color: 'var(--badge-color)', boxShadow: 'var(--badge-shadow)', borderRadius: 'var(--pill-radius)' }}>
          {source}
        </div>
      </div>
